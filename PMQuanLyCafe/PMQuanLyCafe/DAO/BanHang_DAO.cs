...
namespace PMQuanLyCafe.DAO
{
    public class BanHang_DAO
    {
        ...
        {
            string sql = "select HinhAnh,MaDoUong,TenDoUong,GiaTien FROM tblDoUong WHERE TenDoUong LIKE N'%"+ TenDoUong + "%'";
            return DataAccess.ThucThiQuery(sql);
        }
        ...
        {
            string sql = "insert into tblPhieuOrder values ('" + MaPhieuOrder + "','" + NgayOrder + "','" + MaNV + "',NULL,"+ SoBan + ",N'"+ MoTa + "','False')";
            return DataAccess.ThucThiNonQuery(sql);
        }
